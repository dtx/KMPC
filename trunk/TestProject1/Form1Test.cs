...
namespace TestProject1
{
    ...
    public class Form1Test
    {
        ...
        {
            PlayerShortcutForm_Accessor target = new PlayerShortcutForm_Accessor(); // TODO: Initialize to an appropriate value
        ...
        [TestMethod()]
        [DeploymentItem("KMPC.exe")]
        public void OnButtonOKClickedFullTest()
        {
            PlayerShortcutForm_Accessor target = new PlayerShortcutForm_Accessor(); // TODO: Initialize to an appropriate value
            object sender = null; // TODO: Initialize to an appropriate value
            EventArgs e = null; // TODO: Initialize to an appropriate value
            ...
            PlayerShortcutForm_Accessor target2 = new PlayerShortcutForm_Accessor(); // TODO: Initialize to an appropriate value
            target2.textBox_play.Text = "Control + P";
            target2.textBox_stop.Text = "Control + S";
            target2.textBox_full.Text = "Alt + Enter";
            target2.textBox_mute.Text = "F7";
            target2.textBox_fwd.Text = "Shift + Control + F";
            target2.textBox_bwd.Text = "Shift + Control + B";
            target2.textBox_vup.Text = "F9";
            target2.textBox_vdown.Text = "F8";
            target2.textBox_class_name.Text = "WMPlayerApp2";

            target2.OnButtonOKClicked(sender, e);
                ...
                for (int j = 0; j < root.ChildNodes.Count; j++)
                {
                    ...
                    if (child.HasChildNodes)
                    {
                        for (int i = 0; i < child.ChildNodes.Count; i++)
                        {
                            ...
                            {
                                if (i == 0)
                                    Assert.AreEqual(child.ChildNodes[i].InnerText, "WMPlayerApp2");
                                if (i == 1)
                                    Assert.AreEqual(child.ChildNodes[i].InnerText, "Control + P");
                                if (i == 2)
                                    Assert.AreEqual(child.ChildNodes[i].InnerText, "Control + S");
